...
**Decisão do PM (Murilo, 10/09/2026): APROVADO.** As 2 divergências
resolvidas: (1) `.mt-bench-row` mantido como padrão próprio,
documentado deliberadamente — não unificar com PlayerCard (contexto de
uso diferente); (2) tipografia `Rajdhani` do campinho/banco classificada
como BRDATA Extension (mesma lógica do `Bebas Neue` nos escudos —
identidade "placar de estádio"), não migra pra `--m3-display`. Decisão
completa em
https://github.com/muriloalmeida-alt/FitOS/issues/14#issuecomment-5617865996.
Branch mesclada em `main`.
Prioridade: P0
