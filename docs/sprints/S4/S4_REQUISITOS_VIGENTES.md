...
| Resumo da rodada | ✅ migrada `--m3-*` (`S4-B3-006`) | As 2 telas que cobrem o conceito (`#roundResultsOverlay`, pós-jogo; `#rodadaOverlay`, menu) usam `matchCardHTML()` (`S4-B3-005`) no lugar do antigo `.ct-round-result-row` — 3 pontos de código migrados (a própria lista de cada tela + o sub-bloco de Copa do Brasil dentro de `showRoundResultsModal()`), confirmado por inspeção que não havia um 4º ponto em escopo (o 4º usuário de `.ct-round-result-row`, `renderCopa()`, pertence à tela Tabela, fora do escopo). "Destaque do próprio clube" preservado via wrapper externo (`.m3-mc-mine`), sem alterar a definição do MatchCard |
...
| Comparar jogadores | ✅ migrada `--m3-*` (`S4-B4-004`) | Lista de escolha (passo 1) passou a reaproveitar `playerRow()`/PlayerCard — decisão OPOSTA à de Marcação individual, mas justificada: aqui o pool já é sempre da mesma subposição (filtro pré-existente) e o subtítulo já anuncia qual, então o chip por linha seria redundante (lá, lista mista, chip era essencial). Resultado (passo 2): a auditoria achado "novo" tinha 3 tokens exclusivos ainda legados (`.m3-compare-player b`/`.m3-compare-row`/`.m3-compare-val`), migrados na própria regra base (classe exclusiva desta tela, sem risco de vazar). Comportamento (filtro por posição, exclusão do próprio jogador, cálculo de vencedor/custo-benefício) 100% preservado |
