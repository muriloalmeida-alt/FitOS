...
**3 de 6 telas migradas** (Onboarding, Meus esquemas, Marcação
individual) — Eixos táticos não conta como demanda própria (já
coberta). Ordem de execução recomendada pela própria auditoria:
Onboarding → Meus esquemas → Marcação individual → Comparar jogadores
→ Notícias/Eventos → Histórico/Estatísticas (issues #31-#36).
